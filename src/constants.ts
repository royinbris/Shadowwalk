...
2.2. 원문은 절대 수정하지 않는다. 단어는 그대로 두고 공백만 줄여 이어 붙인다.
2.3. 반복, 어색한 표현, 말버릇(uh, um), 오류까지 그대로 유지한다. (의역/문법 교정 금지)
2.4. 모든 문장은 반드시 (m:ss.x) 형태의 타임스탬프로 시작하고 0.1초 단위까지 표기한다.
2.5. 여러 타임스탬프를 하나로 합칠 경우 첫 타임스탬프를 사용한다.
2.6. 의미 단위 마침표(.)가 있다면 문장을 분리하고, "첫 타임스탬프 + (마침표 이전 단어 수 × 0.3초)"로 새 타임스탬프를 0.1초 단위로 계산한다. (U.S., Dr. 등 약어 예외)
...
[중요 제한]
1. 문장 누락 절대 금지한다.
2. 결과물 앞뒤로 어떤 부가 설명, 인사말, 주석도 추가하지 않는다. 오직 결과만 출력한다.
3. 모든 분석은 친절한 영어 강사처럼 작성한다.
